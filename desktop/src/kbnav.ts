...
export function mountKbNav() {
  ...
  window.addEventListener('keydown', e => {
    ...
    if (ctl && !typing && !e.metaKey && !e.ctrlKey) {
      ...
      // lx180(老板 0923 10:07 补缺口):Alt 切歌——desc 承诺过的键位补上(Alt+→下一首,Alt+←上一首)
      if (e.altKey && e.key === 'ArrowRight') { e.preventDefault(); ctl.next(); return; }
      if (e.altKey && e.key === 'ArrowLeft') { e.preventDefault(); ctl.prev(); return; }
    }
