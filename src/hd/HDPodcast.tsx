    ...
    >
      ...
      <HDGrid min={132 * (H.font.sm / 10)}>
        {POD_CHANNELS.map((ch, ci) => {
          ...
          return (
            <HDHalo color={haloOf(ch.name)} radius={12}><HDTouch key={ch.id} style={st.card} zoom={1.06} onPress={() => openChannel(ch)} focusStyle={{ borderWidth: 2, borderColor: C.brand, borderRadius: 14 }}>
